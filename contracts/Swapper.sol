...
contract Swapper{
    ...
    function executeOrder(uint _orderId,uint _amountIn) external{
        ...
        (,uint debt)=calcRateAndAmountOutB(_orderId,_amountIn);
    ...
    function calcRateAndAmountOut(uint orderId,uint amountIn) public view returns(uint rate,uint amountOut){
        uint out=orders[orderId].toTokenAmount;
        // uint rate=(out*1e8)/orders[orderId].fromTokenAmount;
        rate=(out*1e8)/orders[orderId].fromTokenAmount;
        amountOut=rate*amountIn;
    }

    function calcRateAndAmountOutB(uint orderId,uint amountIn) public view returns(uint rate,uint amountOut){
        uint a=orders[orderId].fromTokenAmount;
        // uint rate=(out*1e8)/orders[orderId].fromTokenAmount;
        rate=(a*1e8)/orders[orderId].toTokenAmount;
        amountOut=rate*amountIn;
    }
